...
| 5 | 쇼핑 | 공동구매 담기 버튼 | 구매 모달 오픈 | 🔧 |
| 6 | 쇼핑 | 가격 포맷 | 원화 표기 (₩1,000) | ✅ |
| 7 | 챗봇 | 키워드 분류 — 사료 | commerce 탭 이동 버튼 | ✅ |
| 8 | 챗봇 | 키워드 분류 — 구토 | hospital 탭 / 긴급 high | ✅ |
| 9 | 챗봇 | GPT 모드 배지 | 환경변수 없으면 "키워드" 표시 | ✅ |
| 10 | 미니툴 | 나이 계산기 | 출생연도 입력 → 사람 나이 표시 | ✅ |
| 11 | 미니툴 | 사료량 계산기 | 몸무게 입력 → 권장량(g) 계산 | ✅ |
| 12 | 미니툴 | 위험 음식 체크 | "양파" → 🚨 위험 메시지 | ✅ |
| 13 | 미니툴 | 위험 음식 체크 | "닭" → ✅ 안전 메시지 | ✅ |
| 14 | 병원 | 응급 배너 | 증상 버튼 클릭 → 병원 연결 | ✅ |
| 15 | 전체 | PWA 설치 | 홈 화면 추가 → 앱 실행 | ✅ |
| 16 | 전체 | 모바일 반응형 | max-w-[430px] 중앙 정렬 | ✅ |
| 17 | 전체 | 탭 전환 시 채팅 유지 | 다른 탭 갔다 와도 대화 내용 보존 | ✅ |
...
- [ ] 해외직구 관부가세 자동 계산기
- [ ] 공동구매 모집 인원 실시간 현황 표시
- [ ] MiniTools → 체중 변화 그래프 (로컬 스토리지 저장)
